...
HOVER_DOCS: dict[str, str] = {
    ...
    "publish": "**publish** `: events { <e>, ... } to <topic>` | `: to <topic> { field: value } [at <rate>]` | `: <value> to <topic> [at <rate>]`",
    "result": "**result** `: <succeeded|aborted|canceled> <action-server> { field: value }`\n\nAnswers a served goal with the status it completed under.",
    # platform
